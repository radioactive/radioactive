...
(function() {
  var Base, BasicEventEmitter, BasicEventEmitterHandler, CellBasedCache, CellBasedCacheEntry, DEBUG, DEFAULT_LOOP_DELAY, EQUALS, GLOBAL, Iterator, LOOP_ITERATIONS_TO_SURVIVE, Monitor, MonitorListenerProxy, Notifier, NotifierPool, NotifierPoolWithValue, PartialResultMarker, ReactiveEval, ReactiveEvalResult, ReactiveLoop, StackVal, StopSignal, Token, Try, VERSION, WaitSignal, build_cell, build_public_api, compare_semver, conditional_build, debug_error, delay, fork, is_special_error, loop_with_callback, next_tick, rxjs, serial, syncify, tap,
  ...
  WaitSignal = (function(_super) {
    ...
    function WaitSignal() {
      WaitSignal.__super__.constructor.call(this, "WaitSignal");
    }
  ...
  CellBasedCache = (function() {
    function CellBasedCache(opts) {
      var _base, _base1, _base2, _base3, _base4;
      this.opts = opts != null ? opts : {};
      if ((_base = this.opts).hasher == null) {
        _base.hasher = JSON.stringify;
      }
      if ((_base1 = this.opts).poll == null) {
        _base1.poll = 0;
      }
      if ((_base2 = this.opts).ttl == null) {
        _base2.ttl = 0;
      }
      if ((_base3 = this.opts).equals == null) {
        _base3.equals = EQUALS;
      }
      if ((_base4 = this.opts).producer == null) {
        _base4.producer = (function() {
    ...
    CellBasedCache.prototype.get = function(key) {
      var hash, _base;
      hash = this.opts.hasher(key);
      return (_base = this._entries)[hash] != null ? _base[hash] : _base[hash] = (function(_this) {
        return function() {
          var entry;
          entry = new CellBasedCacheEntry({
            producer: function(cb) {
              return _this.opts.producer(key, cb);
            },
            poll: _this.opts.poll,
            equals: _this.opts.equals
          });
          entry.___key = key;
          return entry;
        };
    ...
    CellBasedCache.prototype.reset = function(filter) {};

    return CellBasedCache;

  })();

  CellBasedCacheEntry = (function() {
    function CellBasedCacheEntry(opts) {
      var _base, _base1, _base2;
      this.opts = opts != null ? opts : {};
      if ((_base = this.opts).producer == null) {
        _base.producer = (function() {
          throw new Error('producer is required');
        })();
      }
      if ((_base1 = this.opts).equals == null) {
        _base1.equals = EQUALS;
      }
      if ((_base2 = this.opts).poll == null) {
        _base2.poll = 0;
      }
      this.cell = build_cell(new WaitSignal);
    }

    CellBasedCacheEntry.prototype.get = function() {};

    CellBasedCacheEntry.prototype.reset = function() {};

    return CellBasedCacheEntry;
